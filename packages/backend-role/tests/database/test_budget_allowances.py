...
def _administrator(operation: str, *, request_id: str) -> RequestContext:
    return RequestContext(
        request_id,
...
def test_stale_owner_generation_early_reclaim_and_double_final_are_rejected(
    ...
    with pytest.raises(BudgetError):
        repository.reclaim(
            _system("budget.allowance.reclaim"), lease_id=lease.lease_id, now=NOW
        )
    final = AllowanceFinal(lease.lease_id, NODE, 1, Decimal(3), Decimal(3))
    repository.reconcile(
        _system("budget.allowance.reconcile", actor=NODE), final, now=NOW
    )
        ...
        with psycopg.connect(database_url) as connection:
            connection.execute(
                """INSERT INTO router.budget_allowance_reconciliations (
                   ...
                   ) VALUES (%s, %s, %s, 1, 2, 3, %s, false)""",
                (uuid.uuid4(), lease.lease_id, NODE, NOW),
            )
...
def test_direct_sql_rejects_stale_owner_early_reclaim_and_double_final(
    ...
    with psycopg.connect(database_url, autocommit=True) as connection, pytest.raises(
