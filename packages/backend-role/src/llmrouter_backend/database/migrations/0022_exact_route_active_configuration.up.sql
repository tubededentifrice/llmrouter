...
BEGIN
    ...
    IF NEW.exact_route_id IS NOT NULL AND NOT EXISTS (
        ...
        JOIN router.configuration_price_bindings AS binding
          ON binding.configuration_revision_id = NEW.configuration_revision_id
         AND binding.provider_model_route_id = route.id
        JOIN router.diagnostic_route_authorizations AS diagnostic_authorization
          ON diagnostic_authorization.request_id = NEW.request_id
         AND diagnostic_authorization.service_id = NEW.service_id
         AND diagnostic_authorization.workspace_id IS NOT DISTINCT FROM NEW.workspace_id
         AND diagnostic_authorization.exact_route_id = route.id
         AND diagnostic_authorization.route_configuration_revision_id = route.current_revision
        WHERE route.id = NEW.exact_route_id AND route.state = 'active'
    ) THEN
              ...
              (request.assignment_id IS NULL
               ...
               AND EXISTS (
                   ...
                   WHERE diagnostic_use.request_id = request.request_id
                     ...
                     AND diagnostic_use.workspace_id
                         IS NOT DISTINCT FROM request.workspace_id
                     AND diagnostic_use.exact_route_id =
                         NEW.provider_model_route_id
                     AND diagnostic_use.route_configuration_revision_id =
                         NEW.route_configuration_revision_id
               )
